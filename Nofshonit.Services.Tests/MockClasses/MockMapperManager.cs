...
{
	public class MockMapperManager : Profile, IMapperManager
	{
		...
		public MockMapperManager()
		{
			Init(new MapperConfiguration(cfg =>
			{
				cfg.
				...
				.ForMember(dest => dest.ClubCreditCard, source => source.MapFrom(src => src.ClubCreditCard));
					...
					CreateMap<Crm, ContactUsDTO>()
					.ForMember(dest => dest.Id, source => source.MapFrom(src => src.MemberId))
					.ForMember(dest => dest.FullName, source => source.MapFrom(src => src.MemberName))
					.ForMember(dest => dest.Description, source => source.MapFrom(src => src.CrmDescription));
					...
					CreateMap<ProductDTO, ShopingBasket>()
						...
						.ForMember(dest => dest.Expired, source => source.Ignore())
						.ForMember(dest => dest.FinalPrice, source => source.Ignore())
						.ForMember(dest => dest.Id, source => source.Ignore())
						.ForMember(dest => dest.MemberId, source => source.Ignore())
						.ForMember(dest => dest.ProductBarcode, source => source.Ignore())
						.ForMember(dest => dest.ProductSubType, source => source.Ignore())
						.ForMember(dest => dest.Quantity, source => source.Ignore())
						.ForMember(dest => dest.SeatsStatus, source => source.Ignore())
						.ForMember(dest => dest.XmlParams, source => source.Ignore());
				...
				// Make sure no validation are thrown in Tests!
		...
		public dynamic Map(object source, Type destType)
		{
			if (source != null && destType != null)
				return _mapper.Map(source, source.GetType(), destType);
			return null;
		}
